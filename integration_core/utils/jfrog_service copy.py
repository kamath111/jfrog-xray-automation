...
class JFrogAPIClient:
    ...
    def _make_request(
        ...
        if not response.ok:
            err = (
                f"Encountered an error when making API call to {service_endpoint} with params [{params}],"
                f" with status code: {response.status_code}.\n"
                f"Response: {response.text}"
            )
            if response.status_code in [401, 403]:

                log.error("Token might have expired, use" "correct token")

            log.info("Retrying, if applicable")
            raise exceptions.JfrogAPIError(
                f"Encountered an error when making API call to {service_endpoint},"
                f" with status code: {response.status_code}.\n"
                f"Response: {response.text}"
            )
    ...
    def generate_api_token(self, username, password, scope="member-of-groups:*", expires_in=36000):
        """
